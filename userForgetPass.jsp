	...
	function checkId(){
		...
		var reg=new RegExp("[0-9]{18}|([0-9]{17}X)");
	 	if(!reg.test(idNumber.value)){
	 		$('#idInfo').text("Please input 18 digits id number and check form");;
	 		return false;
	 	}
	 	else{
			$.ajax({
				url:"<%=request.getContextPath()%>/CheckIdNumber",
				type:"POST",
				dataType:"text",
				data:
					{
					...
					success:function(data){
						...
						if(data=="has"){
							...
							$('#idInfo').text("The id number is not actived.");
							return false;
						}
						else{
							$('#idInfo').text("The id number does not exist.");
							return false;
						}
					},
					error:function(){
